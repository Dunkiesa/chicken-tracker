...
- **Sex** — a fixed enumeration: **Hen**, **Rooster**, or **Unknown**. **Laying-eligible** birds are Hens; Roosters and Unknowns are hidden from the egg picker by default (with a "show all" escape hatch) and excluded from production metrics. Eligibility follows the current sex value (correctable).
...
- **Note** — a dated entry in a chicken's chronological **notes log** (e.g. vet visits, medications), comprising free-text **content** together with zero or more attached **note images**. Each note image is **cropped to the region of interest** before being saved — the crop is what gets persisted, not the full upload. The date is always user-supplied; AI never infers it. When an AI provider is configured, images are sent for AI processing in the background; the AI's suggested text auto-appends to the content field and the AI's suggested crop is offered as a starting point — both can be edited, accepted, or rejected. Notes can be added, edited, and deleted; viewers edit/delete their own, admins edit/delete any.
  _Avoid_: journal, log entry, comment
- **Note image** — an image attached to a specific note, **cropped to the region of interest before save**. Scoped to that note only; never appears in the chicken's photo gallery (a gallery photo is *of the chicken*; a note image is *evidence of an event*). While the note is being created or edited, the full uploaded image is held transiently and a crop region (normalized 0–1 over the original dimensions) is chosen by the AI, the user, or both; on save, **only the cropped region is written to disk** and the full image is discarded — the persisted file path therefore points at the cropped image, with no original to revert to. Carries the file path, a thumbnail, the AI processing status (pending/processing/succeeded/failed), the AI's last-suggested text (kept for retry), and the recorded_by user. Lives on the same local image folder as photos, but in a separate `note_images` table. Removed when its note is deleted or the user discards the image before saving the note. The **review modal** is the central UI for reviewing AI results: it shows the image with a **free-form crop rectangle** (no aspect-ratio lock), the AI's suggested text (editable), and a **Resend** button that sends the cropped region to the AI with a **text-only prompt** (no bounding-box request). The modal locks during resend; the new text replaces the old unconditionally. The crop rectangle persists in memory until the note is saved.
  _Avoid_: note attachment, note photo
- **AI provider** — an external system that processes note images. Two kinds are supported interchangeably: a **local** HTTP endpoint serving a vision-language model (e.g. llama.cpp with Gemma 4 12B), or a **subscription** vision-capable API (e.g. OpenAI, Anthropic, Google). Selected per-deployment by the AI config file; the same configurable prompt and the same response contract work for either. The provider is **optional** — without it, the user types note content manually and crops images manually.
- **AI config** — a per-deployment YAML file at `./config/ai.yaml`. Its presence is the on/off signal: if the file does not exist (or fails to parse), AI processing is implicitly disabled. When present it carries `enabled`, `extra_args` (e.g. llama.cpp slot pinning), `api_key`, `url` (any OpenAI-compatible endpoint), and a multi-line `prompt` supporting `${...}` substitutions (e.g. image dimensions injected at call time so the model can return normalized coordinates).
