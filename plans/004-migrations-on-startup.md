...
> in `plans/README.md`.
>
> **Drift check (run first)**: `git diff --stat 0cf488e..HEAD -- src/lib/db.ts src/app/api/`
...
- **Planned at**: commit `0cf488e`, 2026-07-01 (refreshed from `5761d68` — line numbers shifted by plans 002/003)
...
| File | Lines |
|------|-------|
| `src/app/api/chickens/route.ts` | 14, 45 |
...
| `src/app/api/dynamic-lists/[type]/route.ts` | 44, 69 |
...
1. `src/app/api/chickens/route.ts` — remove `runMigrations` from import on line 4, remove calls on lines 14 and 45
2. `src/app/api/chickens/[id]/notes/route.ts` — remove `runMigrations` from import on line 4, remove calls on lines 18 and 50
3. `src/app/api/chickens/[id]/photos/route.ts` — remove `runMigrations` from import on line 4, remove calls on lines 21 and 59
4. `src/app/api/chickens/[id]/photos/[photoId]/route.ts` — remove `runMigrations` from import on line 4, remove calls on lines 26 and 74
5. `src/app/api/chickens/[id]/photos/[photoId]/primary/route.ts` — remove `runMigrations` from import on line 4, remove call on line 24
6. `src/app/api/eggs/route.ts` — remove `runMigrations` from import on line 4, remove calls on lines 14 and 53
7. `src/app/api/dynamic-lists/[type]/route.ts` — remove `runMigrations` from import on line 4, remove calls on lines 44 and 69
8. `src/app/api/admin/users/route.ts` — remove `runMigrations` from import on line 5, remove calls on lines 24 and 47
